...
public class CSVPageWriterTest {
    ...
    public void testWriteArticlesGoodCase() throws Exception {
        ...
        assertNotNull(row);
        assertArrayEquals(row, new String[] {"title1","url1","company1","site1"});
        row = csvReader.readNext();
        assertNotNull(row);
        assertArrayEquals(row, new String[] {"title2","url2","company2","site2"});
        row = csvReader.readNext();
        assertNotNull(row);
        assertArrayEquals(row, new String[] {"title3","url3","company3","site3"});
        row = csvReader.readNext();
        assertNotNull(row);
        assertArrayEquals(row, new String[] {"title4","url4","company4","site4"});
        row = csvReader.readNext();
        assertNull(row);
        csvReader.close();
    }
